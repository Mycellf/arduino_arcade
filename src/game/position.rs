...
impl<const ROW_BITS: u8> GenericPosition<ROW_BITS> {
    ...
    #[must_use]
    pub const fn with_row(self, row: u8) -> Self {
        Self(self.0 & !Self::ROW_MASK | row & Self::ROW_MASK)
    }

    #[must_use]
    pub const fn with_column(self, column: u8) -> Self {
        Self((column & Self::COLUMN_MASK) << ROW_BITS | self.0 & Self::ROW_MASK)
    }

    #[must_use]
    pub const fn nudge_column_saturating(self, offset: i8) -> Self {
        let new_column = self.column().saturating_add_signed(offset);

        self.with_column(if new_column > Self::COLUMN_MASK {
            Self::COLUMN_MASK
        } else {
            new_column
        })
    ...
    #[must_use]
    pub const fn nudge_column_overflowing(self, offset: i8) -> (Self, bool) {
        (
